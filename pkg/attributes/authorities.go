...
func (c Client) UpdateDefinition(attr Attribute) (Attribute, error) {
	// Need to figure out how to handle group by
	args := pgx.NamedArgs{
		"authority":    attr.Authority,
		"name":         attr.Name,
	...
	_, err := c.db.Exec(context.TODO(), `
		UPDATE tdf_attribute.attribute AS attr
		SET 
		state = @state,
		rule = @rule,
		description = @description,
		values_array = @values,
		group_by_attr = @groupByAttr,
		group_by_attrval = @groupByAttrVal
		FROM tdf_attribute.attribute_namespace AS authority 
		WHERE attr.namespace_id = authority.id
		AND authority."name" = @authority
		`, args)
	if err != nil {
		return Attribute{}, err
	}
...
func (c Client) DeleteDefinition(authority string, name string) error {
	args := pgx.NamedArgs{
		"authority": authority,
		"name":      name,
	}
	...
	_, err := c.db.Exec(context.TODO(), `
		DELETE FROM tdf_attribute.attribute
		WHERE namespace_id = (
			SELECT id FROM tdf_attribute.attribute_namespace
			WHERE name = @authority
		)
		AND name = @name
	`, args)
